...
AS
BEGIN
    SELECT id, first_name, last_name, email, username, role FROM cms_users;
end
GO
...
AS
BEGIN
    SELECT TOP (@Limit) * FROM cms_posts ORDER BY updated_at DESC ;
end
...
AS
BEGIN
    SELECT * FROM cms_categories ORDER BY created_at;
end
GO
...
AS
BEGIN
    INSERT INTO cms_post_categories (post_id, category_id) VALUES (@PostID, @CatgeoryID)
end
GO
